...
#     if weights is None:
#         weights = 1/(np.ones(len(gradients))*len(gradients))
#     else:
#         weights = weights/weights.sum()
#     # norms = [torch.norm(grad, p=2, dim=0).item() for grad in gradients]
#     norms = [w*(torch.norm(grad, p=2, dim=0).item()**2) for grad, w in zip(gradients, weights)]
#     d = Aggregators.fedavg_aggregate(gradients, weights)
#     d_norm = torch.norm(d, p=2, dim=0).item()**2
#     diversity = sum(norms)/d_norm
#     return diversity
...
# def gradient_diversity_v2(gradients, global_estimates, weights=None):
#     if weights is None:
#         weights = 1/np.ones(len(gradients))
#     else:
#         weights = weights/weights.sum()
#     norms = [torch.norm(grad, p=2, dim=0).item()**2 for grad in gradients]
#     d_norm = torch.norm(global_estimates, p=2, dim=0).item()
#     diversity = np.sqrt(sum(norms)/len(norms))/d_norm
#     return diversity
